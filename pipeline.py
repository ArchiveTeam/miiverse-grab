...
LUA_SHA1 = get_hash(os.path.join(CWD, 'miiverse.lua'))
...
class WgetArgs(object):
    def realize(self, item):
        wget_args = [
            ...
            "--timeout", "30",
            "--tries", "inf",
            "--domains", "nintendo.net",
            "--span-hosts",
            "--waitretry", "30",
            "--warc-file", ItemInterpolation("%(item_dir)s/%(warc_file_base)s"),
            "--warc-header", "operator: Archive Team",
            "--warc-header", "miiverse-dld-script-version: " + VERSION,
            "--warc-header", ItemInterpolation("miiverse-item: %(item_name)s"),
        ]
        ...
        if item_type == 'user':
            wget_args.extend(['--warc-header', 'miiverse-user: {i}'.format(i=item_value)])
            wget_args.append('https://miiverse.nintendo.net/users/{i}'.format(i=item_value))
            wget_args.append('https://miiverse.nintendo.net/users/{i}/album'.format(i=item_value))
        else:
...
pipeline = Pipeline(
    ...
    GetItemFromTracker("http://%s/%s" % (TRACKER_HOST, TRACKER_ID), downloader,
        VERSION),
    PrepareDirectories(warc_prefix="miiverse"),
    WgetDownload(
        WgetArgs(),
        max_tries=2,
